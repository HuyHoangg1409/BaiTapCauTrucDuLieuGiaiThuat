...
class Node {
	...
	*/
	...
	static Node insert(Node root, int val) {
        ...
        root.ht = Node.ht_hidden(root);
        int balance = Node.BF_hidden(root);
        if (balance == 2) {
            if (val < root.left.val) {
                root = Node.LL_hidden(root);
            } else {
                root = Node.LR_hidden(root);
            }
        } else if (balance == -2) {
            if (val > root.right.val) {
                root = Node.RR_hidden(root);
            } else {
                root = Node.RL_hidden(root);
            }
        }
        return root;
    }
